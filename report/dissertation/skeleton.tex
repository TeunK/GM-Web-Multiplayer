...
\subsection{Network Expectations}
...
In a typical networked game scenario, game clients are described by the server as a set of parameters. These parameters represent the "game state". Clients keep pinging and updating the server with their own state. When the clients and server the game state become desynchronised, fairness is diminished \cite{Fairness_and_Playability} and the user experience of the game may be impaired.
...
Exact implementation details can be found in the included GameMaker application files.
...
\section{Conclusion}
The extension works, and can easily be implemented as its functionality is even more straight-forward than the native GameMaker networking features (which are unsupported by web browsers).

The extension offers a scalable solution when combined with the provided server implementation using Node.js and Socket.io. This is proven by the fact that the number of total concurrently connected clients is insignificant compared to the effect the clients exert on the server by having constant interactions.
...
Short section. Mainly specifying that future improvements are expected at the application-level on the server and client without actually affecting the extension itself.
